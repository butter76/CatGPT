...
class BagFileReader(Sequence[bytes]):
  ...
  def __init__(
    ...
    if separate_limits:
      ...
      fd = os.open(filepath.parent / f'limits.{filepath.name}', os.O_RDONLY)
      try:
...
class BagGlobShardReader(Sequence[bytes]):
  ...
  def __init__(
    ...
    for p in patterns:
      ...
      pp_path = Path(pp)
      matching_files = [str(f) for f in pp_path.parent.glob(pp_path.name)]
...
class BagWriter:
  ...
  def __init__(
    ...
    filepath = Path(filename)
    self._records = filepath.open('wb')
    self._limits = (filepath.parent / f'limits.{filepath.name}').open('wb+')
