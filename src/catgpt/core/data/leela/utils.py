...
# Define mapping from NN index to Leela move
def get_leela_move_from_idx(idx: int) -> str:
    """Convert a move index to Leela's notation.

    Args:
        idx: Move index from training data

    Returns:
        Leela's notation of the move (e.g. 'e2e4')
    """

    assert idx >= 0 and idx < len(_LEELA_MOVES), f"Invalid move index: {idx}"
    return _LEELA_MOVES[idx]
