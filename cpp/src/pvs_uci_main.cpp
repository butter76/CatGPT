/**
 * CatGPT PVS UCI Engine - Main Entry Point
 *
 * This is the main executable for the PVS-based UCI chess engine.
 * It creates a UCIHandler with a PVSSearch implementation backed
 * by TensorRT for neural network inference.
 *
 * Usage: catgpt_pvs [engine_path]
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Create UCI handler with PVSSearch factory
        catgpt::UCIHandler handler([evaluator]() {
            return std::make_unique<catgpt::PVSSearch>(evaluator);
        });
