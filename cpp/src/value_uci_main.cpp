/**
 * CatGPT Value UCI Engine - Main Entry Point
 *
 * This is the main executable for the value-based UCI chess engine.
 * It creates a UCIHandler with a ValueSearch implementation backed
 * by TensorRT for neural network inference.
 *
 * The engine uses 1-move lookahead: for each legal move, it evaluates
 * the resulting position with the value network and selects the move
 * that minimizes the opponent's winning probability.
 *
 * Usage: catgpt_value [engine_path]
...
#include "engine/value_search.hpp"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Create UCI handler with ValueSearch factory
        catgpt::UCIHandler handler([evaluator]() {
            return std::make_unique<catgpt::ValueSearch>(evaluator);
        });
