/**
 * Coroutine-based MCTS Search (AlphaZero / Leela Chess Zero style).
 *
 * This is the async version of MCTSSearch.  The algorithm is identical —
 * SELECT → EXPAND → EVALUATE → BACKPROPAGATE → CALC Q — but every GPU
 * evaluation point is a coroutine suspension.  When evaluate needs the
 * neural network it `co_await`s an EvalAwaitable, suspending the coroutine
 * so the worker thread can run a different game's search.  The GPU thread
 * batches these requests and resumes the coroutines when results are ready.
 *
...
#ifndef CATGPT_SELFPLAY_COROUTINE_MCTS_HPP
...
#endif  // CATGPT_SELFPLAY_COROUTINE_MCTS_HPP
