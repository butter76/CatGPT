/**
 * Stockfish Process Pool — async UCI engine integration for self-play.
 *
 * Manages a pool of Stockfish subprocesses, each on a dedicated I/O thread.
 * Search coroutines submit requests via StockfishAwaitable (mirroring
 * EvalAwaitable), suspending the coroutine so thread-pool threads are never
 * blocked on Stockfish I/O.
 *
...
#ifndef CATGPT_SELFPLAY_STOCKFISH_POOL_HPP
#define CATGPT_SELFPLAY_STOCKFISH_POOL_HPP
...
// ─── Stockfish Request (the coroutine-to-SF bridge) ─────────────────────────

/**
 * A single Stockfish search request.
 *
 * Allocated inside the coroutine frame (via StockfishAwaitable).
...
/**
 * Awaitable that submits a search request to the Stockfish pool
 * and suspends the calling coroutine until the result is ready.
 *
 * Usage inside a coroutine:
 *   MoveResult result = co_await StockfishAwaitable(pool, board);
 */
...
public:
    ...
    // Defined after StockfishPool is complete (below).
...
private:
    StockfishPool& pool_;
    StockfishRequest request_;
};

// ─── StockfishProcess (one subprocess) ──────────────────────────────────────

/**
 * Manages a single Stockfish subprocess with stdin/stdout pipes.
 *
 * Not thread-safe — each instance is owned by exactly one worker thread.
 */
class StockfishProcess {
public:
    StockfishProcess(const std::string& binary_path, int threads, int hash_mb)
        : pid_(-1)
    {
        spawn(binary_path);
        uci_handshake(threads, hash_mb);
    }
    ...
    MoveResult search(const std::string& fen, int nodes, const chess::Board& board) {
        ...
        result.gpu_evals = 0;  // Stockfish doesn't use GPU
        result.iterations = 0;
        return result;
    }
...
private:
    void spawn(const std::string& binary_path) {
        ...
        if (pipe(stdin_pipe) != 0 || pipe(stdout_pipe) != 0) {
            throw std::runtime_error("StockfishProcess: pipe() failed: " +
                                     std::string(strerror(errno)));
        }

        pid_ = fork();
        if (pid_ < 0) {
            throw std::runtime_error("StockfishProcess: fork() failed: " +
                                     std::string(strerror(errno)));
        }
    ...
    void uci_handshake(int threads, int hash_mb) {
        send_line("uci");
        wait_for("uciok");

        send_line("setoption name Threads value " + std::to_string(threads));
        send_line("setoption name Hash value " + std::to_string(hash_mb));
    ...
    void send_line(const std::string& line) {
        ...
        if (written < 0) {
            throw std::runtime_error("StockfishProcess: write() failed: " +
                                     std::string(strerror(errno)));
        }
    }

    /**
     * Read one line from Stockfish's stdout.
    ...
    std::string read_line() {
        while (true) {
            ...
            if (n <= 0) {
                if (n == 0) {
                    throw std::runtime_error("StockfishProcess: EOF on read");
                }
                if (errno == EINTR) continue;
                throw std::runtime_error("StockfishProcess: read() failed: " +
                                         std::string(strerror(errno)));
            }
...
/**
 * Pool of Stockfish processes with an async awaitable interface.
 *
 * Worker threads own one StockfishProcess each and pull requests from
...
public:
    /**
     * @param binary_path    Path to Stockfish binary (or just "stockfish" if on PATH).
     * @param num_processes  Number of concurrent Stockfish subprocesses.
     * @param nodes          Fixed node count for `go nodes`.
     * @param threads_per    UCI Threads option per process.
     * @param hash_mb        UCI Hash option (MB) per process.
     * @param thread_pool    Shared thread pool for resuming coroutines.
     */
    StockfishPool(const std::string& binary_path,
                  int num_processes,
                  int nodes,
                  int threads_per,
                  int hash_mb,
        ...
        , shutdown_(false)
    {
        std::println(stderr, "[StockfishPool] Spawning {} Stockfish processes "
                     "(nodes={}, threads={}, hash={}MB)",
                     num_processes, nodes, threads_per, hash_mb);

        // Launch worker threads (each creates its own StockfishProcess)
        workers_.reserve(num_processes);
        for (int i = 0; i < num_processes; ++i) {
            workers_.emplace_back([this, binary_path, threads_per, hash_mb, i]() {
                worker_loop(binary_path, threads_per, hash_mb, i);
            });
        }

        std::println(stderr, "[StockfishPool] All {} workers ready", num_processes);
    }
    ...
    // Non-copyable, non-movable
    StockfishPool(const StockfishPool&) = delete;
    StockfishPool& operator=(const StockfishPool&) = delete;

    /**
     * Submit a search request.  Called from StockfishAwaitable::await_suspend.
     */
    void submit(StockfishRequest* request) {
    ...
    /**
     * Shut down all workers and Stockfish processes.
     */
    void shutdown() {
...
private:
    void worker_loop(const std::string& binary_path, int threads_per, int hash_mb, int worker_id) {
        // Each worker owns its own Stockfish process
        std::unique_ptr<StockfishProcess> sf;
        try {
            sf = std::make_unique<StockfishProcess>(binary_path, threads_per, hash_mb);
        } catch (const std::exception& e) {
            std::println(stderr, "[StockfishPool] Worker {} failed to start Stockfish: {}",
                         worker_id, e.what());
            return;
        }

        while (true) {
            StockfishRequest* request = nullptr;
            ...
            try {
                ...
                // Try to restart Stockfish
                sf.reset();
                try {
                    sf = std::make_unique<StockfishProcess>(binary_path, threads_per, hash_mb);
                    std::println(stderr, "[StockfishPool] Worker {} restarted Stockfish", worker_id);
                } catch (const std::exception& e2) {
                    std::println(stderr, "[StockfishPool] Worker {} failed to restart: {}",
                                 worker_id, e2.what());
        ...
        // Shutdown the Stockfish process
        if (sf) sf->shutdown();
    }
    ...
    // Request queue
    std::deque<StockfishRequest*> queue_;
...
// ─── StockfishAwaitable::await_suspend ──────────────────────────────────────

inline void StockfishAwaitable::await_suspend(std::coroutine_handle<> h) noexcept {
...
#endif  // CATGPT_SELFPLAY_STOCKFISH_POOL_HPP
