...
public:
    ...
    // Optimistic policy priors (from the optimistic_policy_logit head, warmed
    // at the same alloc temperature). Used to drive exploration during the
    // recursive clamp loop. Empty when not used.
