...
5. Verify `$WORK_DIR/${MODEL}.onnx` exists (or fetch via `ONNX_URL`). `MODEL` defaults to `S4`; set `MODEL=S2` (etc.) to point at `S2.onnx`.
...
The result is a self-contained `$WORK_DIR/catgpt` UCI binary plus `$WORK_DIR/${MODEL}.network` (defaults to `S4.network`).
...
```bash
# Default engine path: $CATGPT_TRT_ENGINE, else ./S4.network
./build/bin/lks_uci

# Or pass it explicitly
./build/bin/lks_uci /path/to/S4.network
```
...
|---|---|---|
| `CATGPT_TRT_ENGINE` | `./S4.network` | Engine path if not passed on the CLI |
...
```bash
./build/bin/trt_benchmark                       # uses $CATGPT_TRT_ENGINE or ./S4.network
./build/bin/trt_benchmark /path/to/S4.network   # or pass the path
```
...
ONNX=/path/to/S4.onnx \
NETWORK_OUT=/path/to/S4.network \
bash scripts/trt.sh
```
